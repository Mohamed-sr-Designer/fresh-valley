...
(function () {
  ...
  const DESC = { hero: "Big headline, rotating word, produce orbs and the hero photo", marquee: "Scrolling brand words on a ribbon", categories: "Six-tile bento of categories", product_rail: "Draggable carousel from a collection", story: "Pinned, stacking cards with crossed-out gifts", boxes: "Box cards that fan in", stats: "Count-up numbers", image_text: "Photo beside copy and a checklist", testimonials: "Two rows of moving reviews", banner: "Full-width photo with a call to action", journal: "Latest journal posts", page_head: "Page title, intro and optional photo", strike_list: "Items that get crossed out", seasons: "Four seasonal photo cards", hscroll: "Horizontal cards that scroll with the page", gallery: "Carousel of moments", compare: "Two-column comparison", quote: "Big statement over a photo", cta: "Call-to-action block with line-art", steps: "Numbered steps", features: "Cards with icons or numbers", areas: "Delivery areas with a photo", contact: "Contact form and details", faq: "Questions and answers", legal: "Policy clauses with a table of contents" };
  ...
  document.body.innerHTML = `
    <header class="te-top">
      <div class="te-top__l"><a class="te-back" href="index.html#/online-store" id="teBack">${icon("back")}Exit</a><span class="te-name"><b>${esc(FV.settings.storeName)}</b><span>Market Day theme</span></span></div>
  ...
  function renderGlobal(L) {
    ...
    L.innerHTML = `<div class="te-set">
      ...
      <section id="g-footer" class="stack" style="gap:10px"><div class="te-set__hd"><span class="te-sec__ic">${icon("list")}</span><h2>Footer</h2></div>
        ${inp("footer.title", "Headline", f.title, { area: true, rows: 2, help: "*word* for olive italic" })}${inp("footer.newsletter_text", "Newsletter text", f.newsletter_text, { area: true })}${inp("footer.blurb", "Brand blurb", f.blurb, { area: true })}
